#include "console.h"
#include "../../src/common/termios.h"
#include "../../src/common/ioctls.h"
...
static struct console_data _console;
static struct console_data *const console = &_console;
...
void console_init()
{
	...
	InitializeSRWLock(&console->rw_lock);
	console->in = in;
	console->out = out;
	...
	SetConsoleMode(in, ENABLE_WINDOW_INPUT);
...
{
	return console->input_buffer_head != console->input_buffer_tail;
}
...
size_t console_read(void *b, size_t count)
{
	...
	AcquireSRWLockExclusive(&console->rw_lock);
...
read_done:
	...
	set_pos(console->x, console->y);
	ReleaseSRWLockExclusive(&console->rw_lock);
	return bytes_read;
}

void console_write(const void *b, size_t count)
{
...
int console_control(int cmd, void *arg)
{
	...
	case L_TCGETS:
	{
		memcpy(arg, &console->termios, sizeof(struct termios));
		r = sizeof(struct termios);
		break;
	}

	case L_TCSETS:
	{
		memcpy(&console->termios, arg, sizeof(struct termios));
		r = 0;
		break;
	}
