...
static struct mm_data *const mm = (struct mm_data *const)MM_DATA_BASE;
...
static void map_entry_range(struct map_entry *e, size_t start_page, size_t end_page)
{
	if (e->f)
	{
		...
		size_t r = e->f->op_vtable->pread(e->f, (char*)GET_PAGE_ADDRESS(start_page), desired_size,
...
int mm_fork(HANDLE process)
{
	...
	for (size_t i = 0; i < SECTION_TABLE_COUNT; i++)
		if (mm->section_table_handle_count[i])
		{
			...
			if (!VirtualAllocEx(process, (LPVOID)(MM_SECTION_HANDLE_BASE + offset), BLOCK_SIZE, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE))
			{
				log_error("mm_fork(): Allocate section table 0x%p failed, error code: %d\n", i, GetLastError());
				return 0;
			}
			if (!WriteProcessMemory(process, (LPVOID)(MM_SECTION_HANDLE_BASE + offset), MM_SECTION_HANDLE_BASE + offset, BLOCK_SIZE, NULL))
			{
...
DEFINE_SYSCALL(mmap2, void *, addr, size_t, length, int, prot, int, flags, int, fd, off_t, offset)
{
	...
	return (uintptr_t)mm_mmap(addr, length, prot, flags, 0, vfs_get(fd), offset);
}
