...
static int socket_sendto(struct socket_file *f, const void *buf, size_t len, int flags, const struct sockaddr *dest_addr, int addrlen)
{
	...
	if (addrlen)
	{
		if ((addrlen = translate_socket_addr_to_winsock((const linux_sockaddr_storage *)dest_addr, &addr_storage, addrlen)) == SOCKET_ERROR)
			return -EINVAL;
...
static int socket_sendmsg(struct socket_file *f, const struct msghdr *msg, int flags)
{
	...
	if (msg->msg_namelen)
	{
		if ((wsamsg.namelen = translate_socket_addr_to_winsock((struct linux_sockaddr_storage *)msg->msg_name, &addr_storage, msg->msg_namelen)) == SOCKET_ERROR)
			return -EINVAL;
